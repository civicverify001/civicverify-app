...
export default function Signup() {
  ...
  var allConsentsChecked = form.agreeAge && form.agreeTerms && form.agreeData;
    ...
    <div style={{ minHeight:'100vh', background:'linear-gradient(135deg,'+C.cream+' 0%,#fff 100%)', display:'flex', alignItems:'center', justifyContent:'center', padding:'40px 16px', fontFamily:'DM Sans,-apple-system,sans-serif' }}>
      ...
      <div style={{ width:'100%', maxWidth:520 }}>
        ...
        <div style={{ background:'#fff',borderRadius:20,padding:'36px 32px',boxShadow:'0 4px 24px rgba(11,37,69,0.06)',border:'1px solid rgba(11,37,69,0.06)' }}>
          ...
          <p style={{ fontSize:14,color:'rgba(11,37,69,0.4)',margin:'0 0 8px' }}>{step===1?'Step 1 of 3: Account Info':step===2?'Step 2 of 3: Location':'Step 3 of 3: Demographics & Consent'}</p>
          ...
          {step===3&&<div>
            <div style={{background:'#0B254506',border:'1px solid rgba(11,37,69,0.06)',borderRadius:10,padding:'12px 14px',marginBottom:20}}><p style={{fontSize:13,color:'rgba(11,37,69,0.5)',margin:0}}>{'\uD83D\uDD12'} Demographics are optional and used for polling accuracy only. Organizations see only aggregated, anonymous results — never individual data.</p></div>
            ...
            <div style={{background:'rgba(11,37,69,0.02)',border:'1px solid rgba(11,37,69,0.08)',borderRadius:12,padding:'16px 16px 12px',marginBottom:16}}>
              ...
              <label style={{display:'flex',alignItems:'flex-start',gap:10,cursor:'pointer',marginBottom:12}}>
                <input type="checkbox" checked={form.agreeAge} onChange={function(e){update('agreeAge',e.target.checked)}} style={{marginTop:2,width:18,height:18,accentColor:C.gold,flexShrink:0}}/>
                <span style={{fontSize:13,color:'rgba(11,37,69,0.55)',lineHeight:1.5}}>I confirm I am <strong style={{color:C.navy}}>at least 18 years old</strong>. <span style={{color:'rgba(11,37,69,0.3)'}}>(Required for identity verification)</span></span>
              </label>

              <label style={{display:'flex',alignItems:'flex-start',gap:10,cursor:'pointer',marginBottom:12}}>
                <input type="checkbox" checked={form.agreeTerms} onChange={function(e){update('agreeTerms',e.target.checked)}} style={{marginTop:2,width:18,height:18,accentColor:C.gold,flexShrink:0}}/>
                <span style={{fontSize:13,color:'rgba(11,37,69,0.55)',lineHeight:1.5}}>I have read and agree to the <a href="/privacy" target="_blank" rel="noopener noreferrer" style={{color:C.gold,fontWeight:600,textDecoration:'underline'}}>Privacy Policy</a> and <a href="/terms" target="_blank" rel="noopener noreferrer" style={{color:C.gold,fontWeight:600,textDecoration:'underline'}}>Terms of Service</a>. This is my only account.</span>
              </label>

              <label style={{display:'flex',alignItems:'flex-start',gap:10,cursor:'pointer',marginBottom:4}}>
                <input type="checkbox" checked={form.agreeData} onChange={function(e){update('agreeData',e.target.checked)}} style={{marginTop:2,width:18,height:18,accentColor:C.gold,flexShrink:0}}/>
                <span style={{fontSize:13,color:'rgba(11,37,69,0.55)',lineHeight:1.5}}>I consent to CivicVerify collecting my demographic data to match me with relevant surveys. Organizations only see <strong style={{color:C.navy}}>anonymous, aggregated results</strong>. I can <strong style={{color:C.navy}}>delete my data anytime</strong>.</span>
              </label>
            </div>
            ...
            <div style={{display:'flex',gap:12}}>
              ...
              <button onClick={handleSignup} disabled={loading || !allConsentsChecked} style={{flex:2,padding:14,background:allConsentsChecked?C.gold:'rgba(11,37,69,0.12)',color:allConsentsChecked?'#fff':'rgba(11,37,69,0.3)',border:'none',borderRadius:12,fontSize:15,fontWeight:600,cursor:allConsentsChecked?'pointer':'not-allowed',opacity:loading?0.6:1,transition:'all 0.2s ease'}}>{loading?'Creating...':'Create Account'}</button>
            </div>
            {!allConsentsChecked && <p style={{textAlign:'center',fontSize:11,color:'rgba(11,37,69,0.25)',marginTop:8}}>Please check all three consent boxes to continue</p>}
          </div>}

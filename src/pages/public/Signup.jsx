// src/pages/public/Signup.jsx — Full demographics + 3-step signup
...
// County data for auto-populate (top counties per state)
...
export default function Signup() {
  ...
  function update(field, value) {
    setForm(function(prev) {
      var next = Object.assign({}, prev, { [field]: value });
      if (field === 'state') { next.county = ''; next.city = ''; }
      return next;
    });
    setError('');
  }
  ...
  async function handleStep1() {
    ...
    if (r1.data && r1.data.length > 0) { setLoading(false); return setError('This phone number is already registered.'); }
    var r2 = await supabase.from('users').select('id').eq('email', form.email.toLowerCase().trim()).limit(1);
    if (r2.data && r2.data.length > 0) { setLoading(false); return setError('This email is already registered.'); }
    setLoading(false);
    setStep(2);
  }
  ...
  async function handleSignup() {
    ...
    setLoading(true);
    var res = await supabase.auth.signUp({ email: form.email.trim().toLowerCase(), password: form.password, options: { data: { full_name: form.fullName.trim() } } });
    if (res.error) { setLoading(false); return setError(res.error.message); }
    if (res.data.user) {
      var profile = {
        id: res.data.user.id, email: form.email.trim().toLowerCase(), full_name: form.fullName.trim(),
        phone: form.phone.replace(/\D/g, ''), role: form.role,
        state: form.state, county: form.county.trim(), city: form.city.trim(), zip: form.zip.replace(/\D/g, '').slice(0, 5),
        ...
        is_verified: false, identity_verified: false
      };
      var r = await supabase.from('users').upsert(profile);
  ...
  function Sel(props) {
    return (
      <select value={props.value} onChange={function(e){update(props.field, e.target.value)}} style={selectStyle}>
      ...
      <div style={{ width: '100%', maxWidth: 520 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 20, padding: '36px 32px', boxShadow: '0 4px 24px rgba(11,37,69,0.06)', border: '1px solid rgba(11,37,69,0.06)' }}>
          ...
          <p style={{ fontSize: 14, color: 'rgba(11,37,69,0.4)', margin: '0 0 8px' }}>
            {step === 1 ? 'Step 1 of 3: Account Info' : step === 2 ? 'Step 2 of 3: Location' : 'Step 3 of 3: Demographics'}
          </p>
          <div style={{ display: 'flex', gap: 6, marginBottom: 24 }}>
            ...
            <div style={{ flex: 1, height: 4, borderRadius: 2, background: step >= 3 ? C.gold : 'rgba(11,37,69,0.06)' }} />
          </div>
          ...
          {step === 1 && <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              <div><label style={labelStyle}>Password <span style={{color:C.red}}>*</span></label><input type="password" value={form.password} onChange={function(e){update('password',e.target.value)}} placeholder="Min 8 characters" style={inputStyle} /></div>
              <div><label style={labelStyle}>Confirm <span style={{color:C.red}}>*</span></label><input type="password" value={form.confirmPassword} onChange={function(e){update('confirmPassword',e.target.value)}} placeholder="Re-enter" style={inputStyle} /></div>
            </div>
            <div style={{ marginBottom: 16 }}><label style={labelStyle}>I am signing up as</label><Sel value={form.role} field="role" options={['citizen','org']} placeholder="Select role" /></div>
          ...
          {step === 2 && <div>
            <div style={{ background: C.gold + '08', border: '1px solid ' + C.gold + '20', borderRadius: 10, padding: '12px 14px', marginBottom: 20 }}>
              <p style={{ fontSize: 13, color: 'rgba(11,37,69,0.5)', margin: 0 }}>{'\uD83D\uDCCD'} Location helps match you with community-relevant polls. Never shared publicly.</p>
            </div>
            <div style={{ marginBottom: 16 }}><label style={labelStyle}>State <span style={{color:C.red}}>*</span></label><Sel value={form.state} field="state" options={US_STATES} placeholder="Select your state" /></div>
            <div style={{ marginBottom: 16 }}><label style={labelStyle}>County</label>
              {counties.length > 0 ?
                <Sel value={form.county} field="county" options={counties} placeholder="Select your county" /> :
                <input value={form.county} onChange={function(e){update('county',e.target.value)}} placeholder="Enter your county" style={inputStyle} />
              }
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              <div><label style={labelStyle}>City <span style={{color:C.red}}>*</span></label><input value={form.city} onChange={function(e){update('city',e.target.value)}} placeholder="e.g., Indianapolis" style={inputStyle} /></div>
              <div><label style={labelStyle}>ZIP Code <span style={{color:C.red}}>*</span></label><input value={form.zip} onChange={function(e){update('zip',e.target.value.replace(/\D/g,'').slice(0,5))}} placeholder="e.g., 46201" maxLength={5} style={inputStyle} /></div>
            </div>
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={function(){setStep(1);setError('')}} style={{ flex: 1, padding: 14, background: 'rgba(11,37,69,0.05)', color: 'rgba(11,37,69,0.5)', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{'\u2190'} Back</button>
              <button onClick={handleStep2} style={{ flex: 2, padding: 14, background: C.gold, color: '#fff', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>Continue {'\u2192'}</button>
            </div>
          </div>}

          {/* STEP 3 — Demographics */}
          {step === 3 && <div>
            <div style={{ background: '#0B254506', border: '1px solid rgba(11,37,69,0.06)', borderRadius: 10, padding: '12px 14px', marginBottom: 20 }}>
              <p style={{ fontSize: 13, color: 'rgba(11,37,69,0.5)', margin: 0 }}>{'\uD83D\uDD12'} Demographics are used only for polling accuracy. All responses are anonymous.</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              <div><label style={labelStyle}>Race / Ethnicity</label><Sel value={form.race} field="race" options={RACE_OPTIONS} placeholder="Select..." /></div>
              <div><label style={labelStyle}>Sex</label><Sel value={form.sex} field="sex" options={SEX_OPTIONS} placeholder="Select..." /></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              <div><label style={labelStyle}>Date of Birth</label><input type="date" value={form.dob} onChange={function(e){update('dob',e.target.value)}} style={inputStyle} />{form.dob ? <p style={{ fontSize: 11, color: 'rgba(11,37,69,0.25)', margin: '4px 0 0' }}>Age: {calcAge(form.dob)}</p> : null}</div>
              <div><label style={labelStyle}>Education</label><Sel value={form.education} field="education" options={EDUCATION_OPTIONS} placeholder="Select..." /></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              <div><label style={labelStyle}>Employment</label><Sel value={form.employment} field="employment" options={EMPLOYMENT_OPTIONS} placeholder="Select..." /></div>
              <div><label style={labelStyle}>Household Income</label><Sel value={form.income} field="income" options={INCOME_OPTIONS} placeholder="Select..." /></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              <div><label style={labelStyle}>Marital Status</label><Sel value={form.marital_status} field="marital_status" options={MARITAL_OPTIONS} placeholder="Select..." /></div>
              <div><label style={labelStyle}>Party Affiliation</label><Sel value={form.party} field="party" options={PARTY_OPTIONS} placeholder="Select..." /></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              <div><label style={labelStyle}>Housing</label><Sel value={form.housing} field="housing" options={HOUSING_OPTIONS} placeholder="Select..." /></div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 20 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13, color: 'rgba(11,37,69,0.5)' }}>
                  <input type="checkbox" checked={form.voter_registered} onChange={function(e){update('voter_registered',e.target.checked)}} style={{ accentColor: C.gold, width: 16, height: 16 }} /> Registered Voter
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13, color: 'rgba(11,37,69,0.5)' }}>
                  <input type="checkbox" checked={form.veteran} onChange={function(e){update('veteran',e.target.checked)}} style={{ accentColor: C.gold, width: 16, height: 16 }} /> Veteran
                </label>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }}>
                ...
                <span style={{ fontSize: 13, color: 'rgba(11,37,69,0.45)', lineHeight: 1.5 }}>I agree to the <span style={{ color: C.gold, fontWeight: 600 }}>Terms of Service</span> and <span style={{ color: C.gold, fontWeight: 600 }}>Privacy Policy</span>. I confirm this is my only account.</span>
              </label>
            </div>

            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={function(){setStep(2);setError('')}} style={{ flex: 1, padding: 14, background: 'rgba(11,37,69,0.05)', color: 'rgba(11,37,69,0.5)', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{'\u2190'} Back</button>
              <button onClick={handleSignup} disabled={loading} style={{ flex: 2, padding: 14, background: C.gold, color: '#fff', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 600, cursor: 'pointer', opacity: loading ? 0.6 : 1 }}>{loading ? 'Creating Account...' : 'Create Account'}</button>
            </div>
          </div>}

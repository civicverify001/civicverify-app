  ...
  return (
    <div style={{ fontFamily: 'DM Sans, sans-serif', maxWidth: 800 }}>
      ...
      {missingFields.length > 0 && (
        <div style={{ background: C.gold + '10', border: '1px solid ' + C.gold + '35', borderRadius: 13, padding: '16px 20px', marginBottom: 22, display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          <span style={{ fontSize: 18, flexShrink: 0, marginTop: 1 }}>&#9888;</span>
          <div>
            <p style={{ fontSize: 14, fontWeight: 700, color: C.navy, margin: '0 0 4px' }}>Complete your profile for more surveys</p>
            <p style={{ fontSize: 13, color: 'rgba(11,37,69,0.5)', margin: '0 0 10px', lineHeight: 1.5 }}>Missing: <strong style={{ color: C.navy }}>{missingFields.join(', ')}</strong></p>
            <button onClick={function() { navigate('/citizen/account'); }} style={{ padding: '8px 16px', background: C.gold, color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              Complete Profile &#8594;
            </button>
          </div>
        </div>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 0, marginBottom: 24, background: 'rgba(11,37,69,0.04)', borderRadius: 11, padding: 4 }}>
        {[
          { k: 'available', l: 'Available', count: available.length },
          { k: 'completed', l: 'Completed', count: done.length },
          { k: 'all',       l: 'All',       count: surveys.length },
        ].map(function(t) {
          var isActive = tab === t.k;
          return (
            <button key={t.k} onClick={function() { setTab(t.k); }}
              style={{ flex: 1, padding: '10px 16px', borderRadius: 8, border: 'none', fontSize: 13, fontWeight: 700, cursor: 'pointer', background: isActive ? '#fff' : 'transparent', color: isActive ? C.navy : 'rgba(11,37,69,0.4)', boxShadow: isActive ? '0 1px 6px rgba(11,37,69,0.1)' : 'none', transition: 'all 0.15s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
              {t.l}
              <span style={{ fontSize: 11, fontWeight: 800, padding: '1px 7px', borderRadius: 10, background: isActive ? C.gold : 'rgba(11,37,69,0.08)', color: isActive ? '#fff' : 'rgba(11,37,69,0.4)' }}>
                {t.count}
              </span>
            </button>
          );
      ...
      {shown.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 20px', background: '#fff', borderRadius: 16, border: '1px solid rgba(11,37,69,0.07)', boxShadow: '0 2px 12px rgba(11,37,69,0.04)' }}>
          ...
          <p style={{ fontSize: 16, fontWeight: 700, color: C.navy, margin: '0 0 8px', fontFamily: font }}>
            {tab === 'completed' ? 'No surveys completed yet' : 'No surveys available'}
          </p>
          <p style={{ fontSize: 14, color: 'rgba(11,37,69,0.4)', margin: 0 }}>
            {tab === 'completed' ? 'Take your first survey to make your voice heard!' : 'New polls matching your profile will appear here'}
          </p>
        </div>
      ) : (
        <div style={{ display: 'grid', gap: 16 }}>
          {shown.map(function(s) {
            ...
            var tags = [s.target_state, s.target_race, s.target_sex, s.target_party, s.target_age_min ? 'Age ' + s.target_age_min + '+' : null].filter(Boolean);

            return (
              <div key={s.id} style={{ background: '#fff', borderRadius: 16, border: '1px solid ' + (isDone ? 'rgba(26,122,60,0.2)' : 'rgba(11,37,69,0.07)'), overflow: 'hidden', boxShadow: '0 2px 14px rgba(11,37,69,0.05)', opacity: isDone ? 0.85 : 1 }}>
                <div style={{ padding: '20px 24px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10, gap: 12 }}>
                    ...
                    {isDone && (
                      <span style={{ fontSize: 11, fontWeight: 700, color: C.green, background: C.green + '12', padding: '4px 10px', borderRadius: 20, whiteSpace: 'nowrap', border: '1px solid ' + C.green + '25', flexShrink: 0 }}>
                        &#10003; Completed
                      </span>
                    )}
                  </div>
                  ...
                  {tags.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
                      {tags.map(function(t, i) {
                        return (
                          <span key={i} style={{ fontSize: 10, fontWeight: 700, color: C.navy, background: 'rgba(11,37,69,0.05)', padding: '3px 9px', borderRadius: 6, border: '1px solid rgba(11,37,69,0.06)' }}>
                            {t}
                          </span>
                        );
                      })}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 18, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 12, color: 'rgba(11,37,69,0.4)', fontWeight: 500 }}>
                    ...
                    {s.target_responses && (
                      <span style={{ fontSize: 12, color: 'rgba(11,37,69,0.4)', fontWeight: 500 }}>
                        &#127919; {s.target_responses.toLocaleString()} target
                      </span>
                    )}
                  </div>
                ...
                {!isDone && (
                  <div style={{ padding: '0 24px 20px' }}>
                    <button
                      onClick={function() { navigate('/citizen/surveys/' + s.id); }}
                      style={{ width: '100%', padding: '13px', background: 'linear-gradient(135deg,' + C.navy + ',#1a3a6e)', color: '#fff', border: 'none', borderRadius: 11, fontSize: 14, fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 14px rgba(11,37,69,0.22)', transition: 'transform 0.1s', letterSpacing: 0.2 }}

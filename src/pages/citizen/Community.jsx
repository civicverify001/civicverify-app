  ...
  return (
    ...
    >
      <div style={{ padding: "18px 20px 14px" }}>
        ...
        {/* Video (shared from CivicReels) */}
        {post.video_url && (
          <div style={{ marginBottom: 14, borderRadius: 14, overflow: "hidden", background: "#000", position: "relative" }}>
            <video
              src={post.video_url}
              controls
              playsInline
              preload="metadata"
              style={{ width: "100%", maxHeight: 400, display: "block", borderRadius: 14 }}
            />
            <div style={{ position: "absolute", top: 8, left: 8, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)", borderRadius: 8, padding: "4px 10px", display: "flex", alignItems: "center", gap: 4 }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#C5960C" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M23 7l-7 5 7 5V7zM1 4.5h14v15H1z" /></svg>
              <span style={{ fontSize: 10, fontWeight: 700, color: "#C5960C", letterSpacing: 0.5 }}>CIVIC REEL</span>
            </div>
          </div>
        )}
...
export default function Community() {
  ...
  const fetchPosts = async (p) => {
    ...
    const { data } = await supabase
      ...
      .select("id, content, created_at, likes_count, dislikes_count, comments_count, survey_tag, image_url, video_url, linked_survey_data, users:user_id(full_name, identity_verified)")
  ...
  const handlePost = async (content, imageUrl) => {
    const { data, error } = await supabase.from("community_posts")
      ...
      .select("id, content, created_at, likes_count, comments_count, image_url, video_url, users:user_id(full_name, identity_verified)").single();

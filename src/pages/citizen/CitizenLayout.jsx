// src/pages/citizen/CitizenLayout.jsx — Phase 7: Original + Polish upgrades
...
export default function CitizenLayout() {
  ...
  // Load notification count (pending debate invites)
  useEffect(function() {
    if (!profile) return;
    async function loadNotifs() {
      var { count } = await supabase.from('debates')
  ...
  return (
    <ToastProvider>
      <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'DM Sans, sans-serif' }}>
        ...
        {/* Sidebar — kept original structure */}
        <aside style={{
          ...
          <div style={{ padding: '24px 20px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              {/* Notification bell */}
              <button onClick={function(){ navigate('/citizen/debates'); }} style={{ position: 'relative', width: 32, height: 32, borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                🔔
                {notifications > 0 && <span style={{ position: 'absolute', top: 4, right: 4, width: 8, height: 8, borderRadius: '50%', background: '#ef4444', border: '2px solid #0B2545' }} />}
              </button>
          ...
          {/* Nav links — kept original inline style approach */}
            ...
            {links.map(function(link) {
              return (
                <NavLink key={link.to} to={link.to} end={link.end || false} onClick={function(){setOpen(false)}}
                  ...
                  {/* Notification badge for debates */}
                  {link.to === '/citizen/debates' && notifications > 0 && (
                    <span style={{ fontSize: 10, fontWeight: 700, color: '#fff', background: '#ef4444', padding: '2px 7px', borderRadius: 10, minWidth: 18, textAlign: 'center' }}>{notifications}</span>
                  )}
                </NavLink>
              );
            })}
          ...
          {/* User info — kept original structure */}
        ...
        {/* Main content — kept original padding exactly */}
        <div style={{ flex: 1, marginLeft: 0, overflowX: 'hidden' }} className="cv-main">
          ...
          <header className="cv-mobile-header" style={{ display: 'none', position: 'sticky', top: 0, zIndex: 30, background: C.navy, padding: '12px 16px', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button onClick={function(){ navigate('/citizen/debates'); }} style={{ position: 'relative', width: 40, height: 40, borderRadius: 10, background: 'rgba(255,255,255,0.08)', border: 'none', color: '#fff', fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              🔔{notifications > 0 && <span style={{ position: 'absolute', top: 6, right: 6, width: 8, height: 8, borderRadius: '50%', background: '#ef4444', border: '2px solid ' + C.navy }} />}
            </button>
          </header>
        ...
        {/* Responsive styles — kept original exactly + added overflow fix */}
        <style>{'\
